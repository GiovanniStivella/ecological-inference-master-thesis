...
# Regression with covariates (geographical location)
covariate_reg <- lm(outcome ~ high_income + area_North + area_Centre, data = synthetic_dataset)
summary(covariate_reg)

covariate_coef <- summary(covariate_reg)$coefficients

rownames(covariate_coef) <- gsub("_", "\\_", rownames(covariate_coef), fixed = TRUE)

covariate_tab <- xtable(
  covariate_coef,
...
print(
  covariate_tab,
  file = "../Paper/Images/covariate_regression_summary.tex",
  include.rownames = TRUE,
  sanitize.text.function = identity
)
