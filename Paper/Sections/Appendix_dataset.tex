The dataset to perform the analysis should include, at the most granular level possible, the share of votes received by the candidate ($\overline{Y}_g$), a partition of the voting population in categories of interest (e.g, ethnic groups, which represent $\overline{X}_g$) and covariates at that same geographical level ($Z_g$).
...
ALARM dataset by \citet{t._kenny2021} combine electoral results at the VTD level previously collected by \citet{vest2020precinct} with data on ethnic composition of precincts collected by \citet{uscensus2020} in the 2020 US census.
...
As in Figure \ref{fig:census-geographical-units}, we can think of three scenarios: VTDs that perfectly align with one CBG, VTDs that align with exactly two or more CBGs, and VTDs that cross the boundaries of CBGs.
...
The third scenario is a bit more complicated and requires a procedure to apportion the socio-economic characteristics of the CBGs that are crossed by the VTD boundaries.
