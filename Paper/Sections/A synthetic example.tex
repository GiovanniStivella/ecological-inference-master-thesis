...
Results are presented in Tab. \ref{tab:naive-regression} and show that $\beta_L$ is estimated to be 0.21, while $\beta_H$ is estimated to be 0.21+0.42 = 0.63.
...
%Another estimate can be obtained by including the region of each precinct as a covariate in the regression, therefore allowing for different intercepts across regions. In this case, we estimate the following regression.

%In that case, we can use Southern low-income voters as reference category and we estimate the following regression:
%\begin{align*}
%\overline{Y_g} & = \beta_H*\overline{X}_g^H + \beta_L*\overline{X}_g^L + \gamma_{North}*Z_{g}^{North} + \gamma_{Centre}*Z_{g}^{Centre} + \epsilon_g.
%\end{align*}
